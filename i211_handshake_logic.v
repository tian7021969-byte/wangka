...
//
//  i211_core_logic.v
//  Intel I211 Gigabit Ethernet Controller - BAR0 Core Register Logic
//  (Replaces bar0_i211_sim.v with driver-handshake fixes)
//
// ===========================================================================
//
//  Overview
...
module i211_handshake_logic (
    ...
    // STATUS is now a combinational wire (not a register)
    // See reg_status wire definition below
    ...
    // ---------------------------------------------------------------
    //  CTRL.RST Auto-Clear Timer
    //  When driver writes CTRL bit 26 (RST) = 1, hardware must auto-clear
    //  after 100 clock cycles. At 62.5MHz -> ~1.6µs.
    //  Driver polls STATUS.PF_RST_DONE which is gated on this timer.
    // ---------------------------------------------------------------
    reg [6:0]  rst_autoclear_cnt;
    wire       rst_active = (rst_autoclear_cnt != 7'd0);

    // ---------------------------------------------------------------
    //  EECD EE_REQ -> EE_GNT: 3 clock cycle delayed grant
    //  Driver writes EE_REQ (bit 7)=1, FPGA sets EE_GNT (bit 8)=1
    //  after exactly 3 clock cycles.
    // ---------------------------------------------------------------
    reg [1:0]  ee_gnt_delay_cnt;
    reg        ee_gnt_pending;
    ...
    //  Hardcoded per user requirement: 32'h80080303
    //    bit  0 = 1  FD (Full Duplex)
    //    bit  1 = 1  LU (Link Up)           *** CRITICAL ***
    //    bit  8 = 1  SPEED[0]
    //    bit  9 = 1  SPEED[1]
    //    bit 19 = 1  GIO_MASTER_EN (SPEED[19:18]=10b for 1000Mbps)
    //    bit 31 = 1  LU_ext (Windows e1r68x64.sys checks this)
    //
    ...
    wire [31:0] reg_status = rst_active ?
        ...
        32'h8028_0303;     // Normal: PF_RST_DONE=1
    ...
    task write_register;
        ...
        begin
            case (dw_offset)
                DW_CTRL: begin
                    // Device Control - bit 26 (RST) auto-clears after ~100ns
                    if (be[0]) reg_ctrl[ 7: 0] <= data[ 7: 0];
                    if (be[1]) reg_ctrl[15: 8] <= data[15: 8];
                    if (be[2]) reg_ctrl[23:16] <= data[23:16];
                    if (be[3]) begin
                        reg_ctrl[31:24] <= data[31:24];
                        // bit 26: Device Reset (RST)
                        // Start 100-cycle countdown timer (~1.6µs at 62.5MHz)
                        // Timer will auto-clear bit 26 when it expires
                        if (data[26]) begin
                            rst_autoclear_cnt <= 7'd100;
                        end
                    end
                end
                ...
                DW_EECD: begin
                    ...
                    //  Windows e1r68x64.sys EE_REQ/EE_GNT 握手:
                    //    bit 6: EE_REQ (driver writes)
                    //    bit 7: EE_GNT (FPGA grants after 3 clock cycles)
                    //
                    //  CRITICAL FIX: 3-cycle delayed grant instead of immediate.
                    //  e1r68x64.sys expects hardware to take a few cycles.
                    // =========================================================
                    if (be[0]) begin
                        reg_eecd[2:0] <= data[2:0];  // SK, CS, DI
                        reg_eecd[6]   <= data[6];     // EE_REQ

                        // === EE_REQ -> EE_GNT: 3 cycle delayed handshake ===
                        if (data[6] && !reg_eecd[6]) begin
                            // Rising edge of EE_REQ -> start 3-cycle grant delay
                            ee_gnt_pending   <= 1'b1;
                            ee_gnt_delay_cnt <= 2'd2;  // 2 more + current = 3 cycles
                        end else if (!data[6]) begin
                            // EE_REQ cleared -> immediately revoke grant
                            reg_eecd[7]      <= 1'b0;
                            ee_gnt_pending   <= 1'b0;
                        end
                    end
                    ...
                    // === 强制保持只读位 ===
                    reg_eecd[8]  <= 1'b1;   // EE_PRES
                    reg_eecd[9]  <= 1'b1;   // AUTO_RD
                    reg_eecd[19] <= 1'b1;   // FLASH_DETECTED_I210
                end
    ...
    always @(posedge clk) begin
        if (!rst_n) begin
            ...
            // RST auto-clear timer init (100-cycle)
            rst_autoclear_cnt <= 7'd0;

            // EE_GNT delayed grant init
            ee_gnt_pending     <= 1'b0;
            ee_gnt_delay_cnt   <= 2'd0;
            ...
            // ---- CTRL.RST 100-Cycle Auto-Clear Timer ----
            // Counts down from 100. When reaching 1, clear CTRL bit 26.
            // This simulates ~1.6µs hardware reset delay at 62.5MHz.
            if (rst_autoclear_cnt > 7'd0) begin
                rst_autoclear_cnt <= rst_autoclear_cnt - 7'd1;
                if (rst_autoclear_cnt == 7'd1) begin
                    reg_ctrl[26] <= 1'b0;  // Auto-clear RST
                end
            end

            // ---- EECD EE_GNT 3-Cycle Delayed Grant ----
            // After EE_REQ rising edge, count down 3 cycles then set EE_GNT.
            if (ee_gnt_pending) begin
                if (ee_gnt_delay_cnt > 2'd0) begin
                    ee_gnt_delay_cnt <= ee_gnt_delay_cnt - 2'd1;
                end else begin
                    reg_eecd[7]    <= 1'b1;   // Grant NVM access (EE_GNT)
                    ee_gnt_pending <= 1'b0;
                end
            end
