...
module i211_core_logic (
    ...
    //  after ~100ns. At 62.5MHz (16ns period), 100ns / 16ns ≈ 7 cycles.
    //  We use an 8-cycle countdown for safety margin.
    // ---------------------------------------------------------------
    reg [3:0]  rst_autoclear_cnt;
    wire       rst_active = (rst_autoclear_cnt != 4'd0);
            ...
            case (dw_offset)
                DW_CTRL: begin
                    ...
                    if (be[3]) begin
                        ...
                        // Start 8-cycle countdown timer (~128ns at 62.5MHz)
                        // Timer will auto-clear bit 26 when it expires
                        if (data[26]) begin
                            rst_autoclear_cnt <= 4'd8;
                        end
                    end
                end
    ...
    always @(posedge clk) begin
        if (!rst_n) begin
            ...
            // RST auto-clear timer init
            rst_autoclear_cnt <= 4'd0;
            ...
            // ---- CTRL.RST Auto-Clear Timer ----
            // Counts down from 8. When reaching 1, clear CTRL bit 26.
            // This simulates ~100ns hardware reset delay at 62.5MHz.
            if (rst_autoclear_cnt > 4'd0) begin
                rst_autoclear_cnt <= rst_autoclear_cnt - 4'd1;
                if (rst_autoclear_cnt == 4'd1) begin
                    reg_ctrl[26] <= 1'b0;  // Auto-clear RST after ~100ns
                end
            end
